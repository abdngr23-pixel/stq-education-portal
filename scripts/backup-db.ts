 ...
 *
 * Requirements:
 * 1. DATABASE_URL is mandatory. No silent localhost fallback.
 * 2. pg_dump absence or failure: EXIT NON-ZERO. No fake SQL. No success message.
 * 3. Verified backup: exit code 0, file exists, file size >= threshold, SHA-256 generated.
 * 4. Redacted credentials only. Never leak passwords in logs.
 * 5. Retention cleanup runs ONLY after newly verified backup succeeds.
 * 6. Preserves existing backups when current backup fails.
 * 7. Zero stale/synthetic SQL placeholders.
 * 8. --dry-run executes zero writes.
 * 9. --verify-only checks DATABASE_URL, pg_dump binary, and target connectivity.
 */
...
/**
 * Parses DATABASE_URL strictly.
 * Throws if URL is missing, invalid, or lacks required connection parameters.
 */
export function parseDatabaseUrl(url?: string): ParsedDbConfig {
  if (!url || typeof url !== 'string' || url.trim() === '') {
    throw new Error('DATABASE_URL is mandatory. No silent localhost fallback is permitted.');
  }
  ...
  if (!parsed.hostname) {
    throw new Error('DATABASE_URL must specify a valid hostname.');
  }
...
export function redactDatabaseUrl(url?: string): string {
  ...
  try {
    ...
    return `${prefix}${userPart}${parsed.hostname}${portPart}${parsed.pathname}${searchPart}`;
  } catch {
 ...
 */
export function runRetentionCleanup(backupDir = BACKUP_DIR, retentionDays = RETENTION_DAYS, isDry = false): { cleanedCount: number; preservedCount: number } {
...
export async function runBackup(args: string[] = process.argv.slice(2)): Promise<void> {
  ...
  if (!databaseUrl) {
    console.error('❌ ERROR FATAL: DATABASE_URL tidak didefinisikan dalam environment.');
    console.error('Pencadangan database produksi DIBATALKAN demi keamanan.');
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`❌ ERROR FATAL saat mem-parsing DATABASE_URL: ${errorMsg}`);
    process.exit(1);
  }

  const redactedUrl = redactDatabaseUrl(databaseUrl);
  console.log(`[1] Target Host     : ${dbConfig.host}:${dbConfig.port}`);
  console.log(`[2] Target Database : ${dbConfig.database}`);
  console.log(`[3] Target User     : ${dbConfig.user || '[DEFAULT]'}`);
  console.log(`[4] Redacted Target : ${redactedUrl}`);
  console.log(`[5] Direktori Arsip : ${BACKUP_DIR}`);
  console.log(`[6] Retensi Hari    : ${RETENTION_DAYS} hari`);
  console.log(`[7] Mode Operasi    : ${isVerifyOnly ? 'VERIFIKASI SAJA (--verify-only)' : isDryRun ? 'SIMULASI (--dry-run)' : 'PRODUKSI'}\n`);
  ...
  if (!pgDumpCheck.available) {
    if (!isDryRun) {
      ...
      if (isVerifyOnly) {
        console.error('Hasil --verify-only: GAGAL (pg_dump tidak tersedia).');
      }
      process.exit(1);
    } else {
  ...
  if (isVerifyOnly) {
    ...
    console.log('=====================================================');
    console.log('- DATABASE_URL: Valid dan terkonfigurasi');
    console.log(`- pg_dump: Tersedia (${pgDumpCheck.version})`);
    console.log('- Konektivitas: Host database dapat dihubungi');
    return;
  }
  ...
  if (isDryRun) {
    ...
    console.log(`Command simulasi     : pg_dump -h ${dbConfig.host} -p ${dbConfig.port} -U ${dbConfig.user} -d ${dbConfig.database} -F p -f "${backupFilePath}"`);
