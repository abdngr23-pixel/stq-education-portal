...
| **TR-TAHF-02** | **Tasmi'/Sima'an Reward Issuance**: Authoritative reward issuance is restricted to Mudir (GLOBAL) and Kabid Tahfizh (DOMAIN) only. Ordinary MT, PH, ADM, and operational staff (`PETUGAS_OPERASIONAL_TAHFIZH`) are strictly denied (`CAPABILITY_NOT_GRANTED`). Prior target grant to POT with `ASSIGNED_UNITS` is formally SUPERSEDED per DIR-2026-023. *(M3.2 UAT #11)* | `TAHFIZH` | `TahfizhReward` (target) / `AuditLog` | `KABID_TAHFIZH`, `MUDIR` | `tahfizh.reward.issue` | `DOMAIN` (Kabid), `GLOBAL` (Mudir) | `authorizeCanonical(user, 'tahfizh.reward.issue', ctx)`: Evaluates domain/global position grant; POT fails closed | `santriId`, `halaqohId` | `lib/auth/canonical-evaluator.ts`, `app/actions/tahfizh.ts` | `tests/milestone3-2-uat-business-rules.test.ts` (Sec 10.2: Mudir/Kabid ALLOW) | `tests/milestone3-2-uat-business-rules.test.ts` (Sec 10.1: ordinary MT DENY; Sec 10.3: POT DENY `CAPABILITY_NOT_GRANTED`) | `UAT-TAHF-02` | `EVIDENCE_UAT_TAHF_02.json` (reward issuance record + audit sink) | **NOT_READY** *(Pending C2C provisioning)* |
