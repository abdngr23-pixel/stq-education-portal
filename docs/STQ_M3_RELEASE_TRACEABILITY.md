...
6. **Academic PositionCapability Classification & Operational Boundary**:
   ...
   - `ACADEMIC_UNIT_CONTAINMENT = BLOCKED_TECHNICAL` (`EducationSession` / `TeachingAssignment` have no authoritative `OrgUnit` relation; failing closed without borrowing halaqoh/kamar into academic `orgUnitIds`).
   - `STUDI_UMUM_ATTENDANCE = DEFERRED / DENY / NOT ACTIVATED` (Kepesantrenan attendance uses `HADIR/IZIN/SAKIT/ALFA`, no `MASBUK`; Studi Umum attendance remains formally deferred).
