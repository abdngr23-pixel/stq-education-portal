...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **TR-ACAD-01** | **Academic Schedule Read**: Teachers view scheduled teaching sessions for their assigned classes and subjects. *(M3.2 UAT #5, M3.3B, PR #29 / DIR-2026-028)* | `AKADEMIK` | `EducationSession`, `TeachingAssignment`, `EducationCohort`, `Subject` | `GURU_KEPESANTRENAN` (Approved `PERSONAL` contract) / `SUBJECT` account (Studi Umum) | `academic.schedule.read` | `GLOBAL` (Kepesantrenan); subject-bound (Studi Umum) | `authorizeCanonical({ capability: 'academic.schedule.read', resourceContext: { educationSessionId } })` + `checkSchemaReadiness()`. GLOBAL capability scope does not bypass server-side teacher/session ownership. | `educationSessionId` | `lib/server/pendidikan-v2-service.ts` (`PendidikanV2Service.getEducationSessions`) | `tests/milestone3-3c1-real-postgres.test.ts` (Sec 4.2) | `tests/milestone3-3c1-real-postgres.test.ts` ("missing capability => DENY") | `UAT-ACAD-01` | `EVIDENCE_UAT_ACAD_01.json` (schedule response + query log) | **NOT_READY** *(Pending C2C position/assignment provisioning & C2D activation)* |
| **TR-ACAD-02** | **Start Teaching Session & Teacher Attendance**: Authenticated teacher begins scheduled session via atomic CAS. Serves as authentic teacher attendance. Badal/substitute strictly fails closed. *(M3.2 UAT #6, M3.3B, M3.3C1, PR #29 / DIR-2026-028)* | `AKADEMIK` | `EducationSession` (`status`, `scheduledStaffId`, `actualTeacherStaffId`, `startedAt`, `updatedAt`) | `GURU_KEPESANTRENAN` (Approved `PERSONAL` contract) / `SUBJECT` account (Studi Umum) | `academic.session.start` | `GLOBAL` (Kepesantrenan); subject-bound (Studi Umum) | `PendidikanV2Service.startEducationSession`: CAS status update (`status = SCHEDULED` $\rightarrow$ `updateMany` $\rightarrow$ `STARTED`; count === 1) + substitute guard (`authoritativeScheduledStaffId === staffId`) + server feature flag check. GLOBAL scope does not remove teacher session ownership. | `educationSessionId` | `lib/server/pendidikan-v2-service.ts` (`PendidikanV2Service.startEducationSession`) | `tests/milestone3-3c1-real-postgres.test.ts` (Sec 4.2) | `tests/milestone3-3c1-real-postgres.test.ts` (Sec 4.1: substitute mismatch => `SUBSTITUTE_TEACHER_POLICY_NOT_APPROVED`) | `UAT-ACAD-02` | `EVIDENCE_UAT_ACAD_02.json` (session start transition + audit log) | **NOT_READY** *(Pending C2C provisioning & C2D activation)* |
| **TR-ACAD-03** | **Record Learning Materials**: Authenticated actual teacher records curriculum material (`materi`) on active session with forensic audit log. *(M3.3B, PR #29 / DIR-2026-028)* | `AKADEMIK` | `EducationSession` (`materi`, `materiRecordedAt`, `materiRecordedByUserId`, `updatedAt`), `AuditLog` | `GURU_KEPESANTRENAN` (Approved `PERSONAL` contract) / `SUBJECT` account (Studi Umum) | `academic.material.record` | `GLOBAL` (Kepesantrenan); subject-bound (Studi Umum) | `PendidikanV2Service.recordSessionMaterial`: Active session check (`status === STARTED`) + actual teacher ownership check (`actualTeacherUserId === actorUserId`) + single transaction update and audit sink | `educationSessionId` | `lib/server/pendidikan-v2-service.ts` (`PendidikanV2Service.recordSessionMaterial`) | `tests/milestone3-3b-pendidikan-foundation.test.ts` (materials record success) | `tests/milestone3-3b-pendidikan-foundation.test.ts` (record on `SCHEDULED` or `COMPLETED` session => `DENY`) | `UAT-ACAD-03` | `EVIDENCE_UAT_ACAD_03.json` (materials update diff + audit event) | **NOT_READY** *(Pending C2C provisioning & C2D activation)* |
| **TR-ACAD-04** | **Kepesantrenan Student Attendance**: Teacher records student presence with canonical statuses (`HADIR`, `IZIN`, `SAKIT`, `ALFA`). Strictly NO MASBUK. Students must be enrolled in roster. **Track Boundary**: Applies strictly to Kepesantrenan flow under `GURU_KEPESANTRENAN` PERSONAL contract. Studi Umum student attendance throws `STUDI_UMUM_ATTENDANCE_POLICY_DEFERRED` (formally deferred; SUBJECT accounts hold zero attendance authority). *(M3.2 UAT #6, M3.3B, PR #29 / DIR-2026-028)* | `AKADEMIK` | `EducationSessionAttendance`, `EducationSessionParticipant` | `GURU_KEPESANTRENAN` (Approved `PERSONAL` contract; strictly excluded from SUBJECT accounts) | `academic.attendance.record` | `GLOBAL` (Kepesantrenan) | `PendidikanV2Service.recordSessionAttendance`: Track boundary guard (`KEPESANTRENAN` allowed; `STUDI_UMUM` $\rightarrow$ `STUDI_UMUM_ATTENDANCE_POLICY_DEFERRED`) + roster participant integrity verification + valid status check + audit log | `educationSessionId`, `santriId` | `lib/server/pendidikan-v2-service.ts` (`PendidikanV2Service.recordSessionAttendance`) | `tests/milestone3-3b-pendidikan-foundation.test.ts` (Kepesantrenan attendance mutation success) | `tests/milestone3-3b-pendidikan-foundation.test.ts` (unrostered santri => `DENY`; `MASBUK` status => rejected; Studi Umum attendance => `STUDI_UMUM_ATTENDANCE_POLICY_DEFERRED`) | `UAT-ACAD-04` | `EVIDENCE_UAT_ACAD_04.json` (attendance batch log + audit diff) | **NOT_READY** *(Pending C2C provisioning & C2D activation)* |
...
| **TR-TAHF-02** | **Tasmi'/Sima'an Reward Issuance**: Authoritative reward issuance target policy is restricted to Mudir (GLOBAL) and Kabid Tahfizh (DOMAIN) only. Ordinary MT, PH, ADM, and operational staff (`PETUGAS_OPERASIONAL_TAHFIZH`) are strictly denied. Prior target grant to POT with `ASSIGNED_UNITS` is formally SUPERSEDED per DIR-2026-023. Runtime server action in `app/actions/reward-sanksi.ts` enforces legacy check with feature-gated shadow parity logging. Production state unverified by this task. *(M3.2 UAT #11)* | `TAHFIZH` | `TahfizhReward` (target) / `AuditLog` | `KABID_TAHFIZH`, `MUDIR` | `tahfizh.reward.issue` | `DOMAIN` (Kabid), `GLOBAL` (Mudir) | Runtime guard in `app/actions/reward-sanksi.ts` (`prosesRewardTasmiSimaanAction`) enforces legacy check (`session.role === "KS" || Boolean(session.isKepalaBidangTahfidz)`); wraps `shadowAuthorizeIfEnabled` for feature-gated parity logging (not live canonical cutover); canonical ABAC evaluation (`authorizeCanonical`) denies POT with `CAPABILITY_NOT_GRANTED` while runtime action returns error message | `santriId`, `halaqohId` | `app/actions/reward-sanksi.ts` (`prosesRewardTasmiSimaanAction`), `lib/auth/canonical-evaluator.ts` | `tests/reward-abac-fail-closed.test.ts` (action success/message for Mudir & Kabid), `tests/milestone3-3c1-real-postgres.test.ts` (Sec 5.1 & 6.1 Mudir GLOBAL & Kabid DOMAIN ALLOW) | `tests/reward-abac-fail-closed.test.ts` (action error message on unauthorized caller); `tests/milestone3-3c1-real-postgres.test.ts` Sec 5.1 (canonical POT DENY `CAPABILITY_NOT_GRANTED`; Sec 6.1 real DB parity) | `UAT-TAHF-02` | `EVIDENCE_UAT_TAHF_02.json` (reward issuance record + audit sink) | **NOT_READY** *(Pending C2C provisioning)* |
...
export const CANONICAL_UAT_CAPABILITIES = [
  "academic.schedule.read",      // TR-ACAD-01 (Approved GURU_KEPESANTRENAN PERSONAL contract & SUBJECT account binding)
  "academic.session.start",       // TR-ACAD-02 (Approved GURU_KEPESANTRENAN PERSONAL contract & SUBJECT account binding)
  "academic.material.record",     // TR-ACAD-03 (Approved GURU_KEPESANTRENAN PERSONAL contract & SUBJECT account binding)
  "academic.attendance.record",   // TR-ACAD-04 (Approved GURU_KEPESANTRENAN PERSONAL contract; Kepesantrenan only, Studi Umum deferred)
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `academic.schedule.read` | **PASS** | **PASS** | **PASS** | **BLOCKED** *(Needs C2B)* | **APPROVED** *(GURU_KEPESANTRENAN / SUBJECT account binding; pending C2C/C2D)* | **NOT_READY** |
| `academic.session.start` | **PASS** | **PASS** | **PASS** | **BLOCKED** *(Needs C2B)* | **APPROVED** *(GURU_KEPESANTRENAN / SUBJECT account binding; pending C2C/C2D)* | **NOT_READY** |
| `academic.material.record` | **PASS** | **PASS** | **PASS** | **BLOCKED** *(Needs C2B)* | **APPROVED** *(GURU_KEPESANTRENAN / SUBJECT account binding; pending C2C/C2D)* | **NOT_READY** |
| `academic.attendance.record`| **PASS** | **PASS** | **PASS** | **BLOCKED** *(Needs C2B)* | **APPROVED** *(GURU_KEPESANTRENAN PERSONAL contract; Studi Umum attendance DEFERRED)* | **NOT_READY** |
