...
| **TR-TAHF-02** | **Tasmi'/Sima'an Reward Issuance**: Authoritative reward issuance is restricted to Mudir (GLOBAL) and Kabid Tahfizh (DOMAIN) only. Ordinary MT, PH, ADM, and operational staff (`PETUGAS_OPERASIONAL_TAHFIZH`) are strictly denied (`CAPABILITY_NOT_GRANTED`). Prior target grant to POT with `ASSIGNED_UNITS` is formally SUPERSEDED per DIR-2026-023. *(M3.2 UAT #11)* | `TAHFIZH` | `TahfizhReward` (target) / `AuditLog` | `KABID_TAHFIZH`, `MUDIR` | `tahfizh.reward.issue` | `DOMAIN` (Kabid), `GLOBAL` (Mudir) | Server action `prosesRewardTasmiSimaanAction` evaluates canonical ABAC (`authorizeCanonical`); POT fails closed with `CAPABILITY_NOT_GRANTED` | `santriId`, `halaqohId` | `app/actions/reward-sanksi.ts` (`prosesRewardTasmiSimaanAction`), `lib/auth/canonical-evaluator.ts` | `tests/reward-abac-fail-closed.test.ts`, `tests/milestone3-3c1-real-postgres.test.ts` (Sec 5.1 & 6.1) | `tests/reward-abac-fail-closed.test.ts` (POT DENY `CAPABILITY_NOT_GRANTED`) | `UAT-TAHF-02` | `EVIDENCE_UAT_TAHF_02.json` (reward issuance record + audit sink) | **NOT_READY** *(Pending C2C provisioning)* |
