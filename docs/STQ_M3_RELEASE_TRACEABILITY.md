...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **TR-ACAD-01** | **Academic & Pesantren Schedule Read**: Teachers must view scheduled teaching sessions for their assigned cohorts/classes and subjects. *(M3.2 UAT #5, M3.3B)* | `AKADEMIK` | `EducationSession`, `TeachingAssignment`, `EducationCohort`, `Subject` | `GURU_AKADEMIK`, `GURU_KEPESANTRENAN` | `academic.schedule.read` | `ASSIGNED_UNITS` | `authorizeCanonical(user, 'academic.schedule.read', ctx)` + schema check | `academicYearId`, `cohortId`, `orgUnitIds` | `lib/server/pendidikan-v2-service.ts` (`getPendidikanV2Schedule`) | `tests/milestone3-3c1-real-postgres.test.ts` (Sec 4.2) | `tests/milestone3-3c1-real-postgres.test.ts` ("missing capability => DENY") | `UAT-ACAD-01` | `EVIDENCE_UAT_ACAD_01.json` (schedule response + query log) | **NOT_READY** *(Pending C2B schema & C2C provisioning)* |
