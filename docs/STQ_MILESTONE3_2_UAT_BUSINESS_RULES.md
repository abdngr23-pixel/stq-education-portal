...
| **3. Search display Nama + Kelas** | Rendered student search results in search modals/cards are simplified to show only **Nama** and **Kelas** (`formatSantriSearchResult`). Underlying searchable fields (NIS, full name) remain searchable. | UI Display Contract | N/A (UI Presentation) | **IMPLEMENTED_IN_PR** | `tests/milestone3-2-uat-business-rules.test.ts` (Section 3) |
...
| **5. Studi Umum vs Kepesantrenan** | Information architecture separates Studi Umum (Matematika Terapan, B. Inggris, B. Indonesia) from Kepesantrenan (Bahasa Arab, Fikih, Tafsir, Tajwid, Aqidah Islamiyah) under Pendidikan. | Navigation / Module Sub-tabs | Domain `AKADEMIK` | **IMPLEMENTED_IN_PR** | `tests/milestone3-2-uat-business-rules.test.ts` (Section 5) |
| **6. Absensi Guru + Santri Kepesantrenan** | Kepesantrenan sessions require distinct attendance records for Guru and Santri (`KEPESANTRENAN_ATTENDANCE_CONTRACT`). Status vocabulary is explicitly TBD / M3.3 BUSINESS DECISION. | `KEPESANTRENAN_ATTENDANCE_CONTRACT` | `AKADEMIK` / `KEPESANTRENAN` | **APPROVED_TARGET_PENDING_TECHNICAL** | `tests/milestone3-2-uat-business-rules.test.ts` (Section 6) |
