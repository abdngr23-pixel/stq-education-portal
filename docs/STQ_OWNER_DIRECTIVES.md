...
- **Acceptance Criteria:** Authenticated session for `musyirfah.putri` reads recap data across all santri; setoran mutations outside assigned halaqoh fail closed; reward issuance for PUTRI operational scope is SUPERSEDED by DIR-2026-023 (runtime action fails closed returning error message; ABAC evaluation in test suites asserts `CAPABILITY_NOT_GRANTED`).
- **Evidence / Reference:** `types/architecture-lock.ts:599`, `docs/STQ_OWNER_DIRECTIVES.md: DIR-2026-023`, `app/actions/reward-sanksi.ts: prosesRewardTasmiSimaanAction`, `tests/reward-abac-fail-closed.test.ts`, `docs/STQ_M3_RELEASE_TRACEABILITY.md` TR-TAHF-01, TR-TAHF-02.
...
- **Affected Domain:** TAHFIZH / REWARD / AUTHORIZATION
- **Implementation Status:** `CODE_COMPLETE` (Enforced in runtime server action `prosesRewardTasmiSimaanAction` in `app/actions/reward-sanksi.ts` via legacy guard `session.role === "KS" || Boolean(session.isKepalaBidangTahfidz)`; shadow canonical evaluation executed via `shadowAuthorizeIfEnabled` for parity comparison and audit logging without live cutover; target policy verified in canonical ABAC tests `tests/reward-abac-fail-closed.test.ts` and `tests/milestone3-3c1-real-postgres.test.ts`)
...
- **Acceptance Criteria:** Runtime action `prosesRewardTasmiSimaanAction` enforces legacy issuer check (returning error message on denial); shadow engine evaluates canonical policy for parity; canonical ABAC test evaluation confirms MUDIR (GLOBAL) and KABID_TAHFIZH (DOMAIN) are allowed while POT, musyirfah.putri, ordinary musyrif, and ADM are denied with assertion code `CAPABILITY_NOT_GRANTED`.
- **Evidence / Reference:** `app/actions/reward-sanksi.ts: prosesRewardTasmiSimaanAction`, `lib/auth/shadow-engine.ts: shadowAuthorizeIfEnabled`, `types/architecture-lock.ts`, `tests/reward-abac-fail-closed.test.ts`, `tests/milestone3-3c1-real-postgres.test.ts` Section 5.1 & Section 6.1.
...
| **P11** | Tasmi'/Sima'an reward issuer: Mudir + Kabid ONLY (Lisa / POT superseded) | `BUSINESS_DECISION_CONFIRMED` | `CODE_COMPLETE` | `BACKEND_AUTHORIZED` | `UI_COMPLETE` | `NOT_LIVE / REQUIRES_FRESH_READ_ONLY_VERIFICATION` | Enforced in `app/actions/reward-sanksi.ts` (`prosesRewardTasmiSimaanAction`) via legacy guard (`session.role === "KS" || Boolean(session.isKepalaBidangTahfidz)`) with feature-gated `shadowAuthorizeIfEnabled` parity evaluation (not live canonical cutover). Mudir (GLOBAL) and Kabid Tahfizh (DOMAIN) are authorized in target policy; POT / `musyirfah.putri`, ordinary MT, PH, ADM strictly denied in canonical ABAC evaluation (`CAPABILITY_NOT_GRANTED` asserted in `tests/reward-abac-fail-closed.test.ts` and `tests/milestone3-3c1-real-postgres.test.ts`). Prior target rule granting POT ASSIGNED_UNITS reward authority is formally SUPERSEDED per DIR-2026-023. Production status unverified by this task. |
